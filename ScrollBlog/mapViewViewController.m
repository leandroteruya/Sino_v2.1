...
-(void)loadPlaces:(NSArray*)ary{
    ...
    NSMutableArray *tempPlaces=[[NSMutableArray alloc] initWithCapacity:0];
     ...
     for (NSDictionary *result in allArray) {
         ...
         MyPlace *place=[[MyPlace alloc] initWithCoordinate:CLLocationCoordinate2DMake([[result objectForKey:@"lat"] doubleValue],[[result objectForKey:@"long"] doubleValue])];
         [place setPostID:[result objectForKey:@"postID"]];
         [place setCurrentTitle:[NSString stringWithFormat:@"%@",[self decodeHTMLCharacterEntities:[result objectForKey:@"title"]]]];
         [place setCurrentSubTitle:[NSString stringWithFormat:@"Lat/long : %@,%@",[result objectForKey:@"lat"],[result objectForKey:@"long"]]];
...
- (void)viewDidLoad
{
    ...
    {
        myMapView.frame = CGRectMake(0, 0, 320, 470);
    ...
    {
        myMapView.frame = CGRectMake(0, 0, 320, 420);
    ...
    [segment setTitle:NSLocalizedString(@"map_segment_1",nil) forSegmentAtIndex:0];
    [segment setTitle:NSLocalizedString(@"map_segment_2",nil) forSegmentAtIndex:1];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	if (buttonIndex == 0) {
        ...
        if([[[AppDelegate instance] getGlobalCategory] isEqualToString:@""])
        {
        [HUD showWhileExecuting:@selector(findNearesMe:) onTarget:self withObject:[NSArray arrayWithObjects:[NSString stringWithFormat:@"lat=%f&long=%f&radius=100000",Currentlatitude,Currentlongtitude], nil] animated:YES];
        }
        else
        {
            [HUD showWhileExecuting:@selector(findNearesMe:) onTarget:self withObject:[NSArray arrayWithObjects:[NSString stringWithFormat:@"lat=%f&long=%f&radius=100000&cat=%@",Currentlatitude,Currentlongtitude,[[AppDelegate instance] getGlobalCategory]], nil] animated:YES];
        }
        ...
        if([[[AppDelegate instance] getGlobalCategory] isEqualToString:@""])
        {
            [HUD showWhileExecuting:@selector(findNearesMe:) onTarget:self withObject:[NSArray arrayWithObjects:[NSString stringWithFormat:@"lat=%f&long=%f&radius=50000",Currentlatitude,Currentlongtitude], nil] animated:YES];
        }
        else
        {
            [HUD showWhileExecuting:@selector(findNearesMe:) onTarget:self withObject:[NSArray arrayWithObjects:[NSString stringWithFormat:@"lat=%f&long=%f&radius=50000&cat=%@",Currentlatitude,Currentlongtitude,[[AppDelegate instance] getGlobalCategory]], nil] animated:YES];
        }
        ...
        if([[[AppDelegate instance] getGlobalCategory] isEqualToString:@""])
        {
            [HUD showWhileExecuting:@selector(findNearesMe:) onTarget:self withObject:[NSArray arrayWithObjects:[NSString stringWithFormat:@"lat=%f&long=%f&radius=25000",Currentlatitude,Currentlongtitude], nil] animated:YES];
        }
        else
        {
            [HUD showWhileExecuting:@selector(findNearesMe:) onTarget:self withObject:[NSArray arrayWithObjects:[NSString stringWithFormat:@"lat=%f&long=%f&radius=25000&cat=%@",Currentlatitude,Currentlongtitude,[[AppDelegate instance] getGlobalCategory]], nil] animated:YES];
        }
...
-(void)findNearesMe:(NSArray*)ary{
    
    findNearestArray = [[AppDelegate instance] findNearestMe:[ary objectAtIndex:0]];
    
    NSMutableArray *tempPlaces=[[NSMutableArray alloc] initWithCapacity:0];
    
    
    for (NSDictionary *result in findNearestArray) {
        
        MyPlace *place=[[MyPlace alloc] initWithCoordinate:CLLocationCoordinate2DMake([[result objectForKey:@"lat"] doubleValue],[[result objectForKey:@"long"] doubleValue])];
        [place setPostID:[result objectForKey:@"postID"]];
        [place setCurrentTitle:[NSString stringWithFormat:@"%@",[self decodeHTMLCharacterEntities:[result objectForKey:@"title"]]]];
        [place setCurrentSubTitle:[result objectForKey:@"distance"]];
        [place addPlace:place];
        [tempPlaces addObject:place];
    }
...
- (void)mapView:(MKMapView *)mapView
 annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control {
    ...
    [[AppDelegate instance] getPostByID:se];
    }
}
